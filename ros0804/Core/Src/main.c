...
void DCmotor(float, float, float, float);
void Servo(int, int, int, int);
/* USER CODE END PFP */
...
void Servo(int speed1, int speed2, int speed3, int speed4)
{
	htim10.Instance->CCR1 = speed1;
	htim12.Instance->CCR1 = speed2;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  HAL_Delay(200);
	  HAL_UART_Receive(&huart3, Rx_data, 10, 10);
	  if(USART_RX_Pin&0x8000){
		  for(int t=0; t<4; t++){
			  dc.data[t]=Rx_data[t];
			  servo.data[t]=Rx_data[t+4];
		  }
	  }
	  float speed = dc.d*16;
	  DCmotor(speed, speed, speed, speed);
	  float servospeed = servo.d;
